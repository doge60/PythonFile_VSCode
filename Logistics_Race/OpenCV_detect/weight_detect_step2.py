...
# 2. 掩膜阈值定义
lower_weight = np.array([0, 95, 0])
upper_weight = np.array([179, 215, 92])
# 3. 结构元素定义
        ...
        for i in range(6):
            ...
            if weight_circles is not None:
                ...
                if len(filter_circle) > 0:
                    for weight_data in filter_circle:  
                        weight_dataPack.append(weight_data)
                        # 在原图上绘制检测到的圆
                        cv2.circle(
                            color_image, (weight_data[0], weight_data[1]), weight_data[2], (0, 255, 255), 2)  # 线条颜色为黄色
                        # 在圆心处绘制标记点
                        cv2.circle(color_image, (weight_data[0], weight_data[1]), 2, (255, 255, 0), 2)  # 颜色为青色
                    ...
                    if not contours_weight:
                        ...
                        if contours_weight[max_id_weight].size < 10:
                            ...
                            if radius_weight > 10:
                                ...
                                weight_data[0]=x_weight
                                weight_data[1]=y_weight
                                weight_data[2]=radius_weight
                                weight_dataPack.append(weight_data)
                                # 在图像上绘制拟合出的圆
